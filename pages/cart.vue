<template>
    <div class="bg-[#070707f0] text-white">
        ...
        <main class="container mx-auto p-6 md:p-24 mb-6">
            <div v-if="info !== null && info.carts.length !== 0" class="border-t-2 w-full">
                <table class="w-full border-b-2">
                    <thead class="bg-[#272626e8] text-center">
                        <tr>
                            <th class="w-[24%] p-4">品項</th>
                            <th class="w-[20%] p-4 hidden sm:table-cell">單價</th>
                            <th class="w-[24%] p-4">數量</th>
                            <th class="w-[20%] p-4">小計</th>
                            <th class="w-[10%] p-4"></th>
                        </tr>
                    </thead>
                </table>
                <div class="max-h-[50vh] md:max-h-[60vh] overflow-y-auto [scrollbar-gutter:stable]">
                    <table class="w-full">
                        <tbody class="text-center">
                            <tr class="border-b-2" v-for="(item, index) in info.carts" :key="item.id">
                                <td class="w-[24%] p-2 align-middle">
                                ...
                                <td class="w-[20%] p-2 hidden sm:table-cell align-middle">
                                    NT${{ item.product.price }}
                                </td>
                                <td class="w-[24%] p-2 align-middle">
                                    <div class="border-2 rounded-lg flex items-center justify-center">
                                        <button
                                ...
                                <td class="w-[20%] p-2 align-middle">NT${{ item.total }}</td>
                ...
                <p class="text-right p-6 text-3xl border-t-2"><strong>總計：</strong> NT${{ info.final_total }}</p>
            </div>
...
<script setup lang="ts">
    ...
    const updateQty = async (item: CartItem, qty: number, index: number) => {
        ...
        try {
            ...
            info.value = (await getCartInfo()).data;
        } catch (error) {
    ...
    const addNum = (item: CartItem, index: number) => updateQty(item, item.qty + 1, index);

    const minusNum = (item: CartItem, index: number) => updateQty(item, Math.max(1, item.qty - 1), index);
    ...
    const restrictNumericInput = (e: Event) => {
        ...
        target.value = String(Math.max(1, parseInt(cleaned, 10)));
    };

    const changeNum = async (e: Event, item: CartItem, index: number) => {
        const target = e.target as HTMLInputElement;
        itemIndex.value = index;
        const cleaned = target.value.replace(/\D/g, '');
        ...
        await updateQty(item, num, index);
    };
                ...
                newCarts.forEach((item) => {
                    if (item.qty < 1) {
                        ...
                        updateCartItem(item.id, {
                            data: { product_id: item.product_id, qty: 1 },
